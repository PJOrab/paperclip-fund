...
TRIAGE_SYSTEM = (
    ...
    "[OUTLOOK:] TAG: when an item text contains '[OUTLOOK: ...]', it means forward guidance "
    "or financial outlook was extracted from the filing. This is the most important part of "
    "the item — always include the OUTLOOK content verbatim in the cluster's 'why' field. "
    "Presence of [OUTLOOK:] upgrades importance by +1 (e.g. importance 4 → 5). "
    "Category='earnings' when [OUTLOOK:] is present in an 8-K or 6-K item. "
...
def triage_user(items: list[dict], max_clusters: int = 12) -> str:
    ...
    return (
        ...
        f"[OUTLOOK:] TAG: items containing '[OUTLOOK: ...]' have extracted forward guidance. "
        f"ALWAYS include the OUTLOOK text verbatim in the cluster 'why'. "
        f"Presence of [OUTLOOK:] upgrades cluster importance by +1 (e.g. 4→5). "
        f"Category='earnings' when [OUTLOOK:] is present in an 8-K or 6-K. "
        ...
        "Category by content, importance 2-4 (never 5 — primary sources earn 5). "
        "SHORT INTEREST: items with source 'yahoo_short_interest' show each ticker's FINRA-snapshot "
        "short interest: direction tag (RISING/COVERING/FLAT/ELEVATED), % of float, MoM share-count "
        "change, and days-to-cover (shortRatio). Include when: (a) %float ≥8% (elevated, squeeze-setup "
        "risk on any positive catalyst), (b) MoM repositioning ≥20% (RISING = new bearish bet, "
        "COVERING = squeeze in progress / forced buy-back), or (c) days-to-cover ≥4 (concentrated "
        "squeeze fuel — even at modest %float, thin liquidity makes a covering rush violent). "
        "Category='insider_trade' (positioning signal), tickers=[named ticker]. "
        "Importance 4 for RISING + ≥10% float (gamma-squeeze setup), 4 for COVERING + ≥8% float "
        "(squeeze actively unwinding — late but still actionable), 3 for elevated (5-10%) or "
        "days-to-cover-only triggers, 2 for trend-only. CROSS-REFERENCE: pair every short-interest "
        "item with the same ticker's options_market signal — high short + low P/C ratio + negative "
        "IV skew = textbook gamma-squeeze setup (importance 5 worthy). "
        ...
        "EPS REVISIONS: items with source 'eps_revisions' show sell-side analyst estimate-revision "
        "velocity per ticker (# analysts up/down 7d/30d + numerical EPS drift vs 30d ago) — the IBES/"
        "StarMine factor, academically the strongest single forward-return signal. Emitted ONLY when "
        "directionally clear, so any item present already passed the filter. Include POSITIVE signals "
        "(broad upward revisions + EPS estimate rising) as a tailwind/confirmation cluster; NEGATIVE "
        "signals (broad downward revisions + estimate cutting) as a real warning that the consensus is "
        "repricing the company. Category='earnings' (this is consensus-EPS data). Importance: 4 when "
        "drift ≥7% with aligned 30d direction (NOW-style major repricing), 3 when standard directional, "
        "2 only for borderline. CROSS-REFERENCE: pair POSITIVE revisions with any forward-guidance / "
        "[OUTLOOK:] item on the same ticker (revisions confirm the company guide is being absorbed); "
        "pair NEGATIVE revisions with insider sales / short interest if present (compounding bearish). "
        "Always carry the consensus Q-EPS number into the cluster 'why' so downstream analyst stage has it. "
        "ANALYST CONSENSUS: items with source 'analyst_consensus' show sell-side aggregated price-target "
        "+ recommendation consensus per ticker — the VALUATION complement to eps_revisions (which is "
        "estimate momentum). Three orthogonal signals layered: (1) implied upside/downside vs current "
        "price (PT-Mittel vs Spot %), (2) high-low PT dispersion = analyst disagreement, (3) 3m rating-"
        "bucket drift (net upgrades/downgrades over a quarter, Womack 1996 forward-return signal). "
        "Direction tag in the headline (BULLISH / BEARISH / DISPERSION / UPGRADES / DOWNGRADES) is the "
        "primary read. Category='sentiment' (sell-side opinion, not confirmed fact). Importance tiering: "
        "implied upside/downside ≥30% with rating-mean ≤1.5 OR ≥3.5 = importance 4 (street has a strong "
        "valuation anchor far from spot); ≥20-30% upside/downside = 3; pure dispersion or drift trigger "
        "without big upside = 2-3. CROSS-REFERENCE: pair BULLISH analyst_consensus with POSITIVE "
        "eps_revisions on the same ticker (street raising both estimates AND PTs = strong tailwind); "
        "BEARISH analyst_consensus + NEGATIVE eps_revisions = high-conviction bear setup (e.g. ARM "
        "-24% implied with falling EPS = compression risk); BULLISH PT with DOWNGRADES drift = tension "
        "(PTs lag rating cuts — directional ambiguity, lower conviction); HIGH DISPERSION (Spread "
        "≥100%) on a stock approaching earnings = setup for outsized post-earnings move (pair with "
        "options_market high IV skew + earnings_calendar). Always carry implied upside %, # analysts, "
        "and the rating direction into the cluster 'why' so the analyst stage has the valuation anchor. "
        "Never drop a BEARISH (negative implied) analyst_consensus signal — those are rare and "
        "actionable: the street is saying the stock is materially overvalued relative to fundamentals. "
        "GOV CONTRACTS: items with source 'gov_contracts' show US Federal contract awards / activity "
        "per watchlist company from USAspending.gov (official Treasury/SAM data). These are LEADING "
        "indicators — federal obligations appear in USAspending typically 7-14 days BEFORE the "
        "contractor announces them via press release or 8-K. Tickers covered are federal-revenue "
        "concentrated names (PLTR ~55% gov rev, ORCL Oracle Cloud Gov + JWCC, MSFT/AMZN/GOOGL JWCC "
        "cloud, DELL server hardware, ANET networking) — by design only those with measurable direct "
        "prime flow. Category='news' with tickers=[the named ticker]. Importance tiering by SIZE: "
        "≥$500M activity = importance 5 (single-award catalyst, often press-release-worthy); $100M–$500M "
        "= 4; $10M–$100M = 3; $1M–$10M = 2. Aggregate multiple smaller contracts on the SAME ticker "
        "into one cluster (e.g. 'DELL: $42.5M DHS + $19.3M USCIS + $19.1M HHS = $80.9M federal mods "
        "this week'). Always carry: aggregate $ total, prime agency(ies), and 1-2 representative scope "
        "snippets into the cluster 'why' so downstream analyst stage can frame the revenue-mix signal. "
        "CROSS-REFERENCE: pair with the same ticker's POSITIVE EPS revisions (gov pipeline confirms "
        "consensus uptick), with insider buying (mgmt sees the pipeline), and with earnings_calendar "
        "(near-term earnings setup). Treat 'base contract: YYYY' hints as context — what matters is the "
        "RECENT action date, not the parent IDV signing date. Never drop a multi-million federal "
        "contract — it is by definition material to a ticker's near-term revenue. "
        "INSIDER CLUSTER: items with source 'insider_cluster' aggregate ALL open-market (P/S) Form-4 "
        "transactions per ticker over the recent EDGAR window into one dollar-weighted verdict — buy/sell "
        "$ totals, distinct exec count, net flow, and a label (CLUSTER BUY/SELL = multi-exec same side; "
        "NET BUY/SELL = single-exec but ≥0K; MIXED = both sides active). The cluster-buying factor "
        "(Cohen-Malloy-Pomorski) is the strongest insider-activity signal — multiple executives buying in "
        "the same window is a far stronger conviction read than any single Form-4. Always prefer the "
        "cluster item over the individual sec_form4 filings it summarises (treat the singles as "
        "supporting detail, the cluster as the headline). Category='insider_trade', tickers=[named "
        "ticker]. Importance: 4 for CLUSTER BUY or CLUSTER SELL (≥2 execs same side); 3 for NET BUY/SELL "
        "≥M; 2 for smaller NET or MIXED; 4 if the cluster aligns with an earnings/guidance event on the "
        "same ticker (compounding signal). CROSS-REFERENCE: pair CLUSTER BUY with POSITIVE EPS-revisions / "
        "rising short interest (squeeze setup); pair CLUSTER SELL with NEGATIVE EPS-revisions / "
        "deteriorating guidance (compounding bearish). Routine codes (grant/exercise/tax) are explicitly "
        "excluded — only discretionary open-market action counts. "
        "JOB POSTINGS: items with source 'job_postings' show hiring velocity from Greenhouse / Lever "
        "public ATS boards for AI ecosystem companies — total open requisitions, # new in last 7d / "
        "30d, and a composition breakdown (ML/AI, Infra/DC, Sales/GTM) per company. This is a "
        "forward-revenue / capex-direction LEADING indicator months ahead of guidance. Two flavors: "
        "(1) 'direct' kind = ticker is on the watchlist (e.g. PLTR, DDOG) → reads as direct ticker "
        "signal. (2) 'proxy' kind = private AI ecosystem (Anthropic, xAI, Scale AI, Together AI, "
        "SambaNova, Databricks, Mistral) → reads as AI-capex demand proxy for the public infrastructure "
        "chain (NVDA/AMD/AVGO/ANET/VRT/MSFT/GOOGL — these labs are the marginal buyers). Category='news' "
        "(or 'macro' when clustering multiple AI labs together as ecosystem read), tickers=[direct "
        "ticker if 'direct' kind; otherwise list the downstream public infrastructure names the proxy "
        "implies — e.g. ACTIVE HIRING BURST at Anthropic/xAI/Scale AI = NVDA + MSFT/GOOGL/AMZN]. "
        "Importance: 4 when an ACTIVE HIRING BURST flag fires on an AI lab ≥100 open reqs (real "
        "scale-up signal); 4 when a 'direct' ticker shows a Sales/GTM-dominant burst ≥20 new/7d "
        "(forward revenue pipeline buildout); 3 for steady-state composition reads on tickers with "
        "thesis exposure; 2 for routine snapshots without a burst flag. CROSS-REFERENCE: pair a "
        "Sales/GTM hiring burst with POSITIVE EPS revisions and insider buying on the SAME ticker "
        "(triple-confirm forward growth); pair an Infra/DC burst at an AI lab with capex commentary "
        "from MSFT/GOOGL/META on the downstream NVDA/AVGO/ANET demand cluster. Composition shifts "
        "matter as much as totals: a sudden ML/AI burst at a Sales-heavy ATS (e.g. Databricks) "
        "= R&D push, a sudden Sales burst at a Research-heavy ATS (e.g. Anthropic) = monetization "
        "pivot. ROLL UP: prefer ONE aggregated cluster per kind ('AI ecosystem hiring pulse' bundling "
        "the proxy labs together; separate per-ticker clusters only for 'direct' kind names with "
        "actionable signal). Carry the # new/7d figures into the cluster 'why' so downstream analyst "
        "stage can quantify the burst magnitude. "
        "HYPERSCALER XBRL: items with source 'hyperscaler_financials' show structured quarterly "
        "financial velocity for MSFT/GOOGL/AMZN/META/ORCL pulled directly from SEC EDGAR XBRL "
        "(`data.sec.gov/api/xbrl/companyconcept`). Each item carries the most recent reported "
        "discrete quarter's capex, capex YoY %, capex QoQ %, revenue + revenue YoY %, operating "
        "margin + YoY pp delta, and a direction tag (accelerating ≥30%, expanding ≥15%, steady, "
        "decelerating ≤-5%, contracting ≤-15%). This is the SINGLE strongest read on AI-infra "
        "demand: these 5 companies' capex IS the entire NVDA/AMD/AVGO/ANET/VRT/SMCI top-line "
        "thesis — when Big-4-plus-ORCL capex accelerates the demand for the semi-infra chain is "
        "already booked; when it decelerates the pause shows up here a full quarter before any "
        "downstream miss. NVDA has historically moved >10% on a single hyperscaler-capex print. "
        "Category='earnings' (this is filed financial data, not commentary). Importance: 5 when "
        "capex YoY ≥50% (accelerating + outlier datapoint, e.g. ORCL +269% Stargate validation, "
        "GOOGL +107% TPU/GPU build, MSFT +84% OpenAI fleet — print-worthy on its own); 4 when "
        "accelerating/expanding (capex YoY ≥15%) at any of the 5 with positive margin trend "
        "(buyer is healthy + spending); 4 when contracting (capex YoY ≤-15%, the THESIS-KILLER "
        "datapoint — flag immediately as bearish for the entire semi-infra book); 3 for "
        "steady/decelerating mid-tier reads; 2 only for genuinely sideways. Tickers field: name "
        "the reporting hyperscaler AND the downstream semi-infra tickers most exposed (e.g. "
        "MSFT capex accelerating → tickers=[MSFT, NVDA, AVGO, ANET, VRT]; ORCL accelerating → "
        "[ORCL, NVDA, SMCI, DELL]). ROLL UP: prefer ONE 'AI-infra capex cycle' cluster bundling "
        "all 5 hyperscaler datapoints together when they move the same direction (an aligned "
        "accelerating Big-4-plus-ORCL print is the macro confirmation — split out individual "
        "tickers only when one is OUTLIER, e.g. AMZN decelerating while MSFT accelerates = "
        "market-share-shift sub-cluster). CROSS-REFERENCE: pair capex acceleration with POSITIVE "
        "tw_semi_revenue (TSMC/ASE wafer + packaging pull-through downstream of these orders), "
        "with insider_cluster buys at the named NVDA/AVGO/ANET (mgmt sees the same demand), "
        "with earnings_transcript forward-guide raises (qualitative AI-capex commentary "
        "confirming the hard number), and with eps_revisions on the semi-infra names (street "
        "raising into the print). Always carry the capex $ amount, YoY %, and direction tag "
        "into the cluster 'why' so downstream analyst stage has the velocity anchor. Never drop "
        "a capex contraction read — it is the rarest and most actionable signal in the AI book. "
        "FILING SENTIMENT: items with source 'filing_sentiment' show the Loughran-McDonald "
        "tone trajectory of the MD&A section across the last 2-4 10-Q/10-K filings per US "
        "watchlist ticker — the academic textual-sentiment factor (Loughran & McDonald 2011, "
        "JoF) that predicts forward returns, earnings surprises, and volatility BEYOND hard "
        "numbers. Each item is emitted ONLY when the latest filing trips a materiality gate "
        "(net tone ≥1.5σ from prior-quarter mean, OR risk-tone (neg+unc) accelerating ≥20% "
        "QoQ, OR a new local high/low across the 4-quarter window) — so routine MD&A drift is "
        "already filtered out. Headline format: '[Filing-Sentiment · TICKER] FORM MD&A tone "
        "shift — {flags}. Net trajectory YYYY-MM-DD:±X.X‰ → ... → YYYY-MM-DD:±X.X‰. Latest: "
        "neg X.XX%, pos X.XX%, unc X.XX% (N MD&A words).' Per-mille (‰) net = (positive% − "
        "negative%): positive = bullish tone, negative = bearish/cautious tone. Category='earnings' "
        "(the filing is the primary source) with tickers=[the named ticker]. Importance tiering: "
        "5 when the new extreme aligns with an imminent earnings_calendar entry (mgmt is telegraphing "
        "next print); 4 when ≥1.5σ negative shift OR risk-tone accelerating ≥20% QoQ (the "
        "deterioration-before-the-print pattern); 4 when ≥1.5σ positive shift on a ticker with "
        "POSITIVE eps_revisions (confirming inflection); 3 for new local extremes without the "
        "earnings/revision pair; 2 only when notable flags are mild. CROSS-REFERENCE: pair a "
        "negative tone shift with NEGATIVE eps_revisions and insider_cluster SELL on the same "
        "ticker (triple-confirmed pre-deterioration); pair a positive tone shift with POSITIVE "
        "eps_revisions, hyperscaler XBRL capex acceleration (if a hyperscaler), or earnings_transcript "
        "forward-guide raises (textual + structured + transcript all telegraphing the same direction = "
        "the strongest fundamental-confidence read available pre-print). Treat risk-tone acceleration "
        "as the SINGLE most actionable filing_sentiment flag — it is structurally early. Always carry "
        "the trajectory line and the latest neg/pos/unc percentages into the cluster 'why' so the "
        "analyst stage has the longitudinal anchor. Never drop a filing_sentiment item on a watchlist "
        "ticker — by construction it has already passed the σ/QoQ filter. "
        "TAIWAN SEMI REVENUE: items with source 'tw_semi_revenue' show TWSE-mandatory monthly "
        "net revenue for 5 Taiwan semiconductor bellwethers (TSMC 2330, ASE 3711, Hon Hai 2317, "
        "UMC 2303, MediaTek 2454) — legal disclosures published by the 10th of each month, "
        "3-6 weeks BEFORE the corresponding US-listed customer reports its quarter. Quant funds "
        "buy this signal via Bloomberg at five figures/month; we receive it free via TWSE OpenAPI. "
        "Headline format: '[TWSE Monthly Revenue · LABEL] YYYY-MM: NT$X bn — MoM ±X% / YoY ±X% "
        "(YTD YoY ±X%, TAG).' TAG = accelerating (YoY ≥15%), expanding (5-15%), steady, "
        "softening (YoY -5 to -10%), contracting (YoY ≤-10%). ALWAYS include TSMC and ASE items "
        "— they are the hardest leading indicators in the fund: TSMC wafer billings reflect "
        "NVDA/AMD/AVGO H-series pull-through already booked; ASE CoWoS packaging capacity shows "
        "AI-accelerator bottleneck pressure. Hon Hai (AI-server assembly) confirms datacenter "
        "build velocity. MediaTek/UMC are secondary (edge-AI/mature-node proxies). "
        "Category='earnings' (mandatory regulatory filing, rel=0.92). Importance: "
        "TSMC/ASE accelerating or contracting YoY = 5 (primary thesis catalyst — wafer billings "
        "ARE the NVDA demand signal 1-2 quarters early); TSMC/ASE expanding/softening = 4; "
        "Hon Hai accelerating/contracting = 4; steady reads = 3; MediaTek/UMC any direction = 3. "
        "Tickers: TSMC items → [TSM, NVDA, AMD, AVGO, ARM, ASML]; ASE → [NVDA, AVGO, AMD]; "
        "Hon Hai → [NVDA, SMCI, DELL]; UMC/MediaTek → [AMD, QCOM]. CROSS-REFERENCE: pair "
        "TSMC accelerating with hyperscaler XBRL capex acceleration (hard demand confirmation "
        "from both ends of the supply chain — the strongest possible bull read); pair TSMC "
        "contracting with hyperscaler capex deceleration (pre-miss setup for the entire semi "
        "book — flag as differentiated bearish regardless of price action). Always carry the "
        "MoM/YoY/YTD figures and the direction tag into the cluster 'why'. Never drop a TSMC "
        "or ASE item — their monthly revenue IS the supply-chain lead indicator this fund is "
        "built on. "
        "EARNINGS TRANSCRIPT: items with source 'earnings_transcript' carry extracted forward "
        "guidance and AI/capex commentary from the most recent quarterly earnings call for "
        "watchlist tickers via discountingcashflows.com. Hedge funds pay Refinitiv StreetEvents "
        "five figures/month for the same data; we fetch it free. Headline format: "
        "'[Earnings Call · TICKER QN YYYY · tone=TAG] Guide: \"<sentence>\" | AI/capex: \"<sentence>\"' "
        "where TAG = raised / cut / maintained / mixed (n/a when no guidance sentence found). "
        "Always include when tone=raised or tone=cut — these are the highest-conviction "
        "post-print signals in the fund (management explicitly revising their own forward view). "
        "tone=maintained is inclusion-worthy for hyperscaler AI-capex commentary even without "
        "a numerical raise (qualitative colour on GPU demand). Category='earnings', "
        "tickers=[the named ticker]. Importance: tone=raised AND AI/capex snippet confirming "
        "spend = 5 (best-of-breed post-earnings signal); tone=raised without AI/capex = 4; "
        "tone=cut = 5 (imminent downgrade risk); tone=maintained + AI/capex spend colour = 3; "
        "transcript published but no signals extracted = 2 (still surfaces the call occurred). "
        "CROSS-REFERENCE: pair tone=raised with POSITIVE eps_revisions and analyst_consensus "
        "BULLISH (the full confirmation stack); pair tone=cut with NEGATIVE eps_revisions and "
        "insider_cluster SELL (triple-confirmed deterioration); pair AI/capex spend commentary "
        "from hyperscalers with TSMC tw_semi_revenue acceleration (management's spoken commitment "
        "confirmed by the wafer billings already booked). For hyperscaler tickers (MSFT/GOOGL/ "
        "AMZN/META/ORCL): always carry AI-capex commentary verbatim into the cluster 'why' — "
        "this is the qualitative confirmation of the XBRL capex number. "
        "TECHNICAL LEVELS: items with source 'tech_level' show institutional technical triggers "
        "for liquid watchlist tickers — computed from 1y OHLCV (yfinance). Emitted ONLY when a "
        "trigger fires: Golden/Death Cross (50d SMA crosses 200d), 200d SMA breach (±2% band), "
        "52w high/low (within 1%), 50d SMA breach (±2%), RSI-14 extreme (<30 oversold, >70 "
        "overbought), volume spike (>2× 20d-avg), gap open (>±3%). Headline: '[TECH · TICKER] "
        "<trigger description> (as of DATE)'. These are NOT fundamental signals — they are "
        "institutional positioning and momentum anchors. Include when they modify thesis timing "
        "or risk: a NVDA 52w high with RSI overbought = near-term exhaustion risk on a long; "
        "a golden cross + volume spike = institutional confirmation of a trend. Category='sentiment' "
        "(technical signals are positioning/flow, not fundamental). Importance: Golden/Death "
        "Cross or 200d SMA breach = 4 (regime change signals used by CTA and quant funds to "
        "trigger systematic position changes — these move institutional flows); 52w high/low = 3 "
        "(momentum/exhaustion signal); 50d breach = 3; RSI/volume/gap only = 2. Tickers = "
        "[the named ticker]. CROSS-REFERENCE: a Death Cross on a fundamental bear cluster = "
        "compounding bearish (systematic sellers will pile in); a Golden Cross on a thesis long = "
        "+0.03 conviction (CTA flows arriving). NEVER treat a technical signal as a standalone "
        "thesis driver — it amplifies or modifies the timing of a fundamental call. A ticker "
        "sitting at 52w high with RSI >70 is a 'be careful chasing here' flag, not a short "
        "signal in isolation. Carry the specific level (price vs SMA, RSI value, volume multiple) "
        "into the cluster 'why' so the analyst stage has the quantitative anchor. "
        "INSTITUTIONAL 13F HOLDINGS: items with source 'institutional_13f' track quarterly "
        "SEC 13F-HR filings from ~14 AI/Tech-focused institutions (ARK, Tiger Global, Coatue, "
        "Baillie Gifford, Viking Global, etc.). Emitted only for notable changes: new position "
        "opened, full exit, or ≥20% size change vs prior quarter. Format: '[13F · TICKER] "
        "<institution> <change_description>. As of <period> vs <prior_period>.' "
        "This is smart-money flow data — quant funds pay Novus/Bloomberg five figures/yr for "
        "aggregated 13F analytics; SEC publishes the raw XML free. The 45-day filing lag means "
        "the position may have partially changed by publication, but directional intent is "
        "authoritative. Category='flows' (institutional positioning), importance: full exit "
        "or new position ≥$100M = 4 (major allocation decision); ≥20% reduction by a "
        "conviction-concentrated fund = 3; small addition (<$50M) = 2. Tickers = [the named "
        "ticker]. CROSS-REFERENCE: a Coatue/Tiger reduction on a thesis long + Death Cross + "
        "NEGATIVE eps_revisions = compounding triple-confirmed bear signal; a new ARK/Baillie "
        "Gifford position in an under-owned name + Golden Cross = institutional FOMO inflection. "
        "Only include genuinely market-relevant clusters. If little matters, return fewer."
        "SEMI EQUIPMENT BOOKINGS: items with source 'semi_equipment_bookings' track quarterly "
        "SEC XBRL revenue velocity, gross margin, and RPO (remaining performance obligations = "
        "contracted backlog) for AMAT, LRCX, and KLAC — the three companies that collectively "
        "represent ~70% of semiconductor equipment market revenue. Only emitted when a material "
        "threshold is crossed: revenue YoY ≥10%, RPO QoQ ≥10%, or gross margin YoY swing ≥2pp. "
        "The cycle tag in brackets — upcycle / recovering / orders-accelerating / downcycle / "
        "softening / orders-decelerating / stable — summarises direction. "
        "Cycle relevance: AMAT/LRCX/KLAC equipment orders lead TSMC capacity (and therefore NVDA "
        "chip supply) by 6-18 months; a book-to-bill inflection here is a tier-1 leading indicator "
        "for the entire semiconductor book (NVDA, AMD, MU, ANET, VRT). Format: "
        "'[Semi Equipment · Company (TICKER)] Qn FYyyyy: rev $XB YoY +/-Y% / QoQ +/-Z% [tag]; "
        "backlog $XB (QoQ +/-Y%); GM X% (+/-Ypp YoY). hook.' "
        "Category='supply_chain', importance: upcycle/downcycle inflection = 4 (cycle turning "
        "point); recovering/softening trend = 3; stable within trend = 2. "
        "Tickers: AMAT + LRCX + KLAC directly; secondarily NVDA/AMD/MU/ANET/VRT/TSM. "
        "CROSS-REFERENCE: AMAT/LRCX upcycle + tw_semi_revenue TSMC acceleration = "
        "compounding bullish for entire semi supply chain; KLAC backlog surge before hyperscaler "
        "capex print = leading-edge fab expansion imminent (bullish HBM/CoWoS capacity)."
    )
...
ANALYST_SYSTEM = (
    ...
    "EPS REVISIONS CLUSTERS (source='eps_revisions', category='earnings'): Sell-side consensus "
    "EPS estimate-revision velocity per ticker — the IBES estimate-revision factor (PEAD). The "
    "item label is POSITIVE / NEGATIVE direction (current quarter), with 7d + 30d revision counts "
    "and the numerical EPS drift % vs 30d ago. ALWAYS treat this as a leading indicator: analysts "
    "revise estimates BEFORE the next earnings print, BEFORE the next price re-rating. "
    "MAGNITUDE: drift |Δ| ≥ 10% in 30d with no contradictions = HIGH (consensus is materially "
    "repricing the company); |Δ| 5-10% = MEDIUM; |Δ| 3-5% or count-only signal = LOW-MEDIUM. "
    "CONVICTION IMPACT: POSITIVE revisions aligned with a fundamental bull cluster = +0.05 to "
    "+0.10 conviction (sell-side is starting to capitulate to the upside). NEGATIVE revisions "
    "aligned with a bear cluster = +0.05 to +0.10 conviction on the short side. "
    "DIFFERENTIATION: NEGATIVE revisions on a stock that has held its price (estimates falling "
    "but multiple holding) = differentiated bearish (multiple compression risk). POSITIVE "
    "revisions with stock STILL underperforming = differentiated bullish (rotation candidate). "
    "Carry the Q-EPS number and revision count into key_facts as 'consensus Q-EPS $X.XX, "
    "N analysts raised / M cut in last 30d'. Note alignment with FY revisions when present. "
    "ANALYST CONSENSUS CLUSTERS (source='analyst_consensus', category='sentiment'): Sell-side "
    "VALUATION consensus per ticker — aggregated price targets vs current spot price plus rating "
    "drift over a quarter. The complement to eps_revisions: revisions are estimate momentum, this "
    "is the valuation anchor and disagreement profile. Direction tag in the headline is the "
    "primary read (BULLISH = implied upside ≥20%, BEARISH = implied downside, DISPERSION = high-low "
    "spread, UPGRADES/DOWNGRADES = 3m rating-bucket drift). "
    "MAGNITUDE: |implied| ≥35% = HIGH (street values the stock materially off spot — re-rating "
    "catalyst expected or bull/bear thesis cooking); |implied| 20-35% = MEDIUM; pure dispersion or "
    "drift trigger = LOW-MEDIUM unless paired with another signal. "
    "CONVICTION IMPACT: BULLISH (+25%+ implied) + POSITIVE eps_revisions same ticker = +0.05-0.10 "
    "conviction on the long side (street is both raising estimates AND PTs — broad capitulation). "
    "BEARISH (-15%+ implied) + NEGATIVE eps_revisions = +0.05-0.10 conviction on the short side. "
    "DIFFERENTIATION: BULLISH PT with DOWNGRADES (META-style tension — high PTs but rating cuts) = "
    "PTs lag rating action, lower conviction, often a stale-target warning. UPGRADES with negative "
    "implied (CRWD-style: -19% downside but +4 net upgrades) = analysts hedging — recent rating "
    "boost but PTs haven't caught up to price rally — momentum-already-priced read. HIGH dispersion "
    "(Spread ≥100%) approaching an earnings date (cross-ref earnings_calendar) = setup for outsized "
    "post-print move (the street can't agree on fair value; one side will be very wrong). "
    "Carry implied upside %, # analysts, rating mean, and rating drift into key_facts as 'PT-mean "
    "$X.XX vs spot $Y.YY → +Z% implied across N analysts; rating mean R.RR (label); 3m drift ±D'. "
    "HYPERSCALER XBRL CLUSTERS (source='hyperscaler_financials', category='earnings'): Filed-data "
    "capex / revenue / op-margin velocity for MSFT/GOOGL/AMZN/META/ORCL — the 5 buyers whose "
    "AI-infrastructure spending IS the entire NVDA/AMD/AVGO/ANET/VRT/SMCI demand thesis. These "
    "are XBRL-tagged 10-Q/10-K facts, not commentary; treat them as the strictest tier of "
    "fundamental evidence (reliability 0.95). "
    "MAGNITUDE: capex YoY ≥50% on any of the 5 = HIGH (single-print thesis catalyst — e.g. ORCL "
    "+269% Stargate validation, GOOGL +107% TPU/GPU build, MSFT +84% OpenAI fleet); aligned "
    "Big-4-plus-ORCL acceleration (3+ of 5 capex YoY ≥30%) = HIGH macro confirmation for the "
    "entire semi-infra book; capex YoY 15-50% = MEDIUM; steady = LOW; capex contraction "
    "(YoY ≤-15%) at any of the 5 = HIGH (the rarest, most actionable read — flag as bearish "
    "regime shift for NVDA/AVGO/ANET regardless of magnitude). "
    "CONVICTION IMPACT: hyperscaler capex YoY ≥30% (accelerating tag) on long NVDA/AVGO/ANET/"
    "VRT/SMCI thesis = +0.10 conviction (this is the hardest piece of fundamental evidence in the "
    "AI book — the buyers' own filed numbers). Multi-buyer alignment (3+ of 5 accelerating same "
    "quarter) = +0.10 on the entire semi-infra basket. Capex contraction at any of the 5 on a "
    "long-side semi-infra thesis = MUST reduce conviction by ≥0.10 (or flip read direction if "
    "the contraction is broad-based). Margin compression alongside capex acceleration (capex +XX% "
    "BUT OpM -1pp+ YoY) = capex absorption concern (the buyer is overspending into a slower "
    "monetization curve — softer NVDA pricing-power read). "
    "DIFFERENTIATION: capex acceleration at ONE hyperscaler with deceleration at the other 4 = "
    "market-share-shift signal (the accelerating buyer is taking GPU allocation from the rest — "
    "e.g. ORCL accelerating + MSFT steady = OCI taking incremental NVDA share). Capex acceleration "
    "with NEGATIVE tw_semi_revenue (TSMC/ASE softening) = high-conviction differentiated bear "
    "(buyers committing capex but wafer/packaging already cracking — early demand-pull air "
    "pocket; downstream miss likely 1-2 quarters out). Revenue YoY decelerating with capex still "
    "accelerating = growth-into-capex compression (margin warning for the buyer; risk of capex "
    "cuts in subsequent quarters → bearish for the semi-infra book on the lookahead). "
    "Carry capex $, capex YoY %, revenue YoY %, op-margin %, and op-margin YoY pp delta into "
    "key_facts as 'MSFT Q3 FY26 capex $30.9B YoY +84%, rev $82.9B YoY +18%, OpM 46.3% +0.7pp'. "
    "Always carry the period label so downstream knows whether the print is one quarter old vs "
    "this morning's 10-Q. "
    "FILING SENTIMENT CLUSTERS (source='filing_sentiment', category='earnings'): "
    "Loughran-McDonald MD&A tone trajectory across the last 2-4 10-Q/10-K filings per US "
    "watchlist ticker. The signal is intrinsically LONGITUDINAL — a single MD&A's tone score "
    "is nearly useless, but a multi-quarter trajectory reveals when management language tilts "
    "from confident to hedged, from positive to risk-framing, ahead of the hard numbers turning. "
    "Tone shifts of ≥1.5σ vs the prior-quarter mean precede growth misses by 2-3 quarters; "
    "risk-tone (neg+unc) accelerations of ≥20% QoQ precede guidance cuts by 1-2 quarters; "
    "modal-weak drift ('may', 'could' replacing 'will', 'expect') is the earliest read. "
    "These items are pre-filtered through the FilingLanguageAdapter's σ / QoQ / extreme gate, "
    "so any item present already crossed a materiality threshold — DO NOT discount them as noise. "
    "MAGNITUDE: ≥2σ net-tone shift OR ≥40% QoQ risk-tone change = HIGH (rare, textbook "
    "pre-earnings deterioration / inflection setup); 1.5-2σ OR 20-40% QoQ = MEDIUM; new local "
    "extreme without σ/QoQ trip = LOW-MEDIUM (the gate is permissive on extremes). "
    "CONVICTION IMPACT: negative tone shift (net more negative ≥1.5σ) aligned with NEGATIVE "
    "eps_revisions OR insider_cluster SELL on the same ticker = +0.05-0.10 conviction on the "
    "short side (textual + estimate + insider all telegraphing the next print). Positive tone "
    "shift aligned with POSITIVE eps_revisions + hyperscaler/earnings_transcript guide raises = "
    "+0.05-0.10 long-side conviction (the strongest pre-print fundamental confidence read). "
    "Risk-tone acceleration ≥20% QoQ in ISOLATION (no other corroboration) still warrants +0.03 "
    "downside conviction — academically it leads guidance cuts by ~6 months. "
    "DIFFERENTIATION: negative tone shift on a stock STILL trading near highs / positive analyst "
    "consensus = differentiated bearish (the filing language is repricing the company before the "
    "street is — classic Loughran-McDonald edge: the text leaks the deterioration mgmt has not "
    "yet quantified). Positive tone shift on a stock UNDERPERFORMING / negative consensus = "
    "differentiated bullish (mgmt confidence rebuilding before the print rerates the multiple). "
    "Modal-strong rebound after multiple weak quarters = inflection candidate. "
    "Carry the full trajectory line and the latest neg/pos/unc percentages into key_facts "
    "as 'MD&A net: 2025-08-12:-0.9‰ → 2025-11-05:-1.4‰ → 2026-02-10:-2.6‰ (2.1σ deterioration); "
    "latest unc 1.83% (+24% QoQ)'. Always preserve the unit (‰ = per-mille of MD&A word count). "
    "horizon='quarters' is the default (filing language leads price by 1-3 quarters); use 'weeks' "
    "ONLY when the tone shift coincides with an imminent earnings_calendar entry (≤14d). "
    "consensus_view='differentiated' when the filing tone diverges from analyst_consensus / "
    "price action — the textual signal exists specifically because it is NOT yet in numbers. "
    "TAIWAN SEMI REVENUE CLUSTERS (source='tw_semi_revenue', category='earnings'): TWSE-mandatory "
    "monthly net revenue for TSMC/ASE/Hon Hai/UMC/MediaTek — supply-chain lead indicators published "
    "3-6 weeks before US-listed customers report. TSMC is the single highest-authority leading "
    "indicator for the AI-accelerator cycle: its wafer billings ARE the booked NVDA/AMD/AVGO demand. "
    "MAGNITUDE: TSMC YoY ≥30% (accelerating) = HIGH (AI GPU pull-through is locked in for the "
    "next 1-2 quarters; street is undermodeling forward NVDA/AVGO earnings); TSMC YoY ≤-15% "
    "(contracting) = HIGH (demand air pocket forming — the most actionable bearish supply-chain "
    "signal in the fund); TSMC YoY 15-30% = MEDIUM; ASE (CoWoS packaging) accelerating = HIGH "
    "when paired with TSMC acceleration (packaging bottleneck is clearing = AI build can accelerate); "
    "ASE contracting = MEDIUM-HIGH (packaging crunch easing OR demand pulling back — check against "
    "TSMC direction). Hon Hai = MEDIUM (assembly volume proxy). "
    "CONVICTION IMPACT: TSMC accelerating + hyperscaler XBRL capex accelerating = +0.10 conviction "
    "on the entire NVDA/AVGO/ANET book (both ends of the supply chain pointing the same direction "
    "is the highest-confidence fundamental read available). TSMC contracting + hyperscaler capex "
    "decelerating = MUST reduce conviction by ≥0.10 on semi-infra longs (regime shift). "
    "DIFFERENTIATION: TSMC revenue accelerating while NVDA/AMD stock has already priced in the "
    "demand = aligned (re-rating catalyst exhausted, carry from here). TSMC accelerating while "
    "NVDA/AMD stock has lagged (sell-off or consolidation) = differentiated bullish (the supply "
    "chain has already confirmed what the stock price hasn't yet). TSMC contracting while stock "
    "still near highs = differentiated bearish (hard data diverging from price — the best "
    "short-setup indicator in the AI book). Carry MoM %, YoY %, YTD YoY %, and direction tag "
    "into key_facts as 'TSMC 2026-04: NT$263.7bn, YoY +42.3% (accelerating), YTD +38.1%'. "
    "horizon='quarters' for structural demand read; 'weeks' ONLY if a major TSMC print just landed "
    "and a hyperscaler earnings is ≤14d away (the data-to-earnings catalyst chain is compressed). "
    "EARNINGS TRANSCRIPT CLUSTERS (source='earnings_transcript', category='earnings'): Extracted "
    "forward guidance and AI/capex commentary from the most recent quarterly earnings call. This "
    "is the qualitative complement to the hard XBRL numbers — management explains the WHY behind "
    "capex and revenue trajectory, reveals product-cycle colour not in press releases, and gives "
    "forward ranges that XBRL won't show until next quarter's 10-Q. "
    "MAGNITUDE: tone=raised (guidance explicitly raised on the call) = HIGH when the raise is "
    "quantified ('we now expect capex of $X-Y bn') or when AI/capex spending colour is unambiguous "
    "('we are accelerating GPU procurement through 2026'); tone=cut = HIGH (guidance cut on a "
    "call is a sell signal — management is walking back the street's model in real time); "
    "tone=maintained with strong AI/capex colour = MEDIUM (no numerical raise but confirms the "
    "bullish narrative); tone=maintained without AI angle = LOW. "
    "CONVICTION IMPACT: tone=raised at a hyperscaler (MSFT/GOOGL/AMZN/META/ORCL) + AI/capex "
    "commentary = +0.08 conviction on semi-infra longs (the buyer is saying it on the call — "
    "not just filing the capex number). tone=cut at any watchlist name = conviction floor 0.40 "
    "maximum on existing long thesis; reconsider direction. "
    "DIFFERENTIATION: guidance raised on the call BUT stock has already rallied 15%+ into "
    "earnings = aligned (raise priced in). Guidance raised BUT stock sold off post-print (perhaps "
    "EPS missed) = differentiated bullish (the call has a better forward view than the print "
    "reaction implies — market overreacted to one bad number). Guidance cut on a stock at 52w "
    "high with elevated short interest = differentiated bearish (the call catalyzes the covering "
    "reversal). Carry the guidance sentence and AI/capex sentence verbatim into key_facts so "
    "the thesis stage can quote management directly. "
    "TECHNICAL LEVELS CLUSTERS (source='tech_level', category='sentiment'): Institutional "
    "technical triggers from 1y OHLCV for liquid watchlist tickers. These are NOT fundamental "
    "signals — they are momentum, exhaustion, and regime-change anchors used by CTA and quant "
    "funds to trigger systematic position entries and exits. Include ONLY when the technical "
    "signal modifies the timing or risk of an existing fundamental thesis. "
    "MAGNITUDE: Golden/Death Cross or 200d SMA breach = MEDIUM (regime-change signals trigger "
    "systematic quant selling/buying that can overwhelm fundamentals for days-weeks; a Death "
    "Cross on a fundamental long is a timing risk regardless of the thesis quality); 52w high "
    "with RSI >70 = LOW-MEDIUM (overbought / exhaustion risk — don't chase); 52w low with RSI "
    "<30 = MEDIUM (deeply oversold — mean-reversion setup, watch for a catalyst); 50d breach "
    "= LOW-MEDIUM; volume spike alone = LOW. "
    "CONVICTION IMPACT: Golden Cross on a thesis long = +0.03 (CTA flows arriving, technical "
    "confirmation of the trend). Death Cross on a thesis long = -0.03 (systematic selling "
    "headwind; reduce time-horizon from 'quarters' to 'weeks'). Never add more than ±0.05 "
    "from technical signals alone — they are amplifiers, not drivers. "
    "DIFFERENTIATION: technical breakdown (Death Cross, 200d breach) on a stock where the "
    "fundamental thesis is still intact = watch carefully — if the fundamentals haven't "
    "deteriorated, the technical break is a buy-the-dip window (consensus is momentum-selling "
    "what quant models flag as broken). Technical breakout (52w high, golden cross) on a "
    "stock with NEGATIVE eps_revisions = divergence / fade signal (price leading fundamentals "
    "upward on no fundamental improvement = not differentiated bullish, it's a trap). "
    "horizon='days' for gap-open or volume spike (short-lived flow event); 'weeks' for "
    "cross/breach (CTA position-change window); 'quarters' never appropriate for tech signals. "
    "consensus_view='unclear' for standalone technical signals (market regime is the driver, "
    "not a fundamental view). "
    "INSTITUTIONAL 13F FLOWS CLUSTERS (source='institutional_13f', category='flows'): Quarterly "
    "SEC 13F-HR position changes from AI/Tech-focused institutions — the smart-money flow layer. "
    "Data is authoritative (direct SEC primary filing) but 45-day lagged. Treat a position change "
    "as evidence of where high-information institutions stood at quarter-end. "
    "MAGNITUDE: full exit or new position ≥$200M from a high-conviction fund (Tiger, Coatue, "
    "Viking) = HIGH (a fund that runs concentrated books rarely exits by accident — this is a "
    "thesis-level decision); ≥20% reduction ≥$50M = MEDIUM; new small position <$50M = LOW. "
    "CONVICTION IMPACT: large institutional exit on a thesis long = -0.05 (the fund you respect "
    "most just voted against your thesis — raise burden of proof before adding); new position "
    "from a concentrated fund = +0.03 (external validation of the thesis; does NOT replace "
    "fundamental analysis — they may have different entry price or horizon). "
    "DIFFERENTIATION: a Tiger Global exit from NVDA while hyperscaler_financials shows "
    "accelerating capex = differentiated signal (smart money reducing despite strong fundamentals "
    "— asks whether they see execution risk you don't). ARK adding CRWD post-breach = "
    "non-consensus bullish (retail-facing fund buying institutional fear). "
    "horizon='quarters' (13F reflects a prior-quarter decision; the fund may have already "
    "adjusted further). Always name the specific institution and size change in key_facts. "
    "SEMI EQUIPMENT BOOKINGS CLUSTERS (source='semi_equipment_bookings', category='supply_chain'): "
    "Quarterly SEC XBRL revenue velocity + RPO (backlog) + gross margin for AMAT, LRCX, KLAC. "
    "These are the three largest semiconductor equipment companies (~70% of WFE market). "
    "Data is authoritative (primary SEC 10-Q/10-K XBRL filing) and 30-90 days lagged. "
    "SIGNAL INTERPRETATION: "
    "  - Revenue YoY / QoQ: confirmed shipped volume. upcycle (YoY≥20%) = fab capacity "
    "    expansion imminent for TSMC/Samsung → leading indicator for NVDA chip supply tightening. "
    "  - RPO (backlog) QoQ: orders not yet shipped. Rising RPO when revenue is flat = "
    "    demand surge before revenue recognition; falling RPO = order cancellations or digestion. "
    "  - Gross margin YoY: pricing power. Expanding GM in an upcycle = equipment scarcity "
    "    (AMAT/LRCX/KLAC pricing above cost inflation = sell-side consensus too low). "
    "  - Gross margin compression despite rising revenue = mix shift to lower-margin products "
    "    or cost pressure (bearish for equipment sector relative to semi volumes). "
    "MAGNITUDE: upcycle or downcycle inflection (rev YoY crosses ±20% for the first time in "
    "2+ quarters) = HIGH — cycle turning point with 6-18 month lead time on end-market; "
    "recovering/softening direction = MEDIUM; stable = LOW. "
    "CONVICTION IMPACT: AMAT/LRCX upcycle confirmation + rising KLAC RPO = +0.04 on NVDA/AMD/MU "
    "(entire supply chain benefits from leading-edge capacity build); equipment downcycle "
    "= -0.03 on NVDA supply outlook (fewer fabs being built = less advanced packaging capacity). "
    "DIFFERENTIATION: LRCX revenue inflecting positive when memory_capex_plans are still bearish "
    "= contrarian early-cycle (equipment buyers commit before DRAM/NAND ASP recovery is visible). "
    "KLAC-only backlog surge (AMAT/LRCX flat) = process-control expansion without capacity "
    "growth = yield improvement cycle (bullish for mature logic, less bullish for new fab builds). "
    "horizon='quarters' for confirmed revenue trends; 'months' for RPO-only inflections "
    "(may reverse before revenue recognition). Name the specific company and metric in key_facts. "
...
def analyst_user(clusters: list[dict]) -> str:
    ...
    return (
        ...
        "USE the CURRENT MARKET CONTEXT block (provided below) as a secondary source: "
        "if the feed items don't state consensus estimates explicitly but the market context shows "
        "'Consensus PT: $X (Y% upside, N analysts) rec=Z fwdEPS=$W', use those numbers — "
        "e.g. set consensus_anchor='PT $X (N analysts, rec=Z), fwdEPS=$W'. "
        "If no consensus estimate is available from either source, set consensus_anchor='unknown'. "
...
THESIS_SYSTEM = (
    ...
    "CONSENSUS ANCHOR USAGE: when the upstream analyst provides a `consensus_anchor` with specific numbers "
    "(e.g. 'consensus EPS $0.89, rev $44.6B'), USE THOSE NUMBERS to calibrate your thesis. "
    "The `edge` field should reference the specific consensus estimate being challenged: "
    "'Street expects DC rev $16.5B but [OUTLOOK: guide] implies $18B+ — model systematically too low.' "
    "If consensus_anchor='unknown', note that the differentiation claim lacks a specific baseline — "
    "this is a signal to lower is_differentiated or lower conviction slightly. "
...
DEVIL_SYSTEM = (
    ...
    "BLIND_SPOT QUALITY: the blind_spot field must name ONE concrete thing the bull thesis "
    "is ignoring or underweighting. Good: 'AMD MI300X adoption curve in hyperscaler data centers "
    "not yet in the model — if AMD wins 15%+ DC share in H2, NVDA demand thesis weakens'. "
    "Bad: 'competition risk', 'macro uncertainty', 'valuation'. If the exit_trigger is vague, "
    "use blind_spot to flag it: 'exit_trigger too vague — propose: DC guide < $5.5B = exit'. "
...
def editor_user(triage: dict, theses: list[dict], critiques: list[dict],
    ...
    return (
        ...
        "  CONVICTION THRESHOLD: only include calls with conviction ≥ 0.40. "
        "Calls with conviction < 0.40 belong in 👀 Beobachten, not Top-Calls. "
        "If all theses are below 0.40, write 'Kein Top-Call heute — Conviction zu niedrig.' and skip the section.\n\n"
        "<b>1) TICKER — Long/Short · Conviction X,XX</b>\n"
        "  HEADER FORMAT: if consensus data provides a PT upside %, append it: "
        "'NVDA — Long · Conviction 0.62 · PT +14%↑ (Street)'. "
        "If PT upside is negative vs current price, show '· PT -5%↓ (Street)'. Omit if no consensus PT available.\n"
        ...
        "  SZENARIEN FORMAT: extract % from each scenario's `target` field "
        "(e.g. '$260 (+19%)' → '+19%', '$180 (-17%)' → '-17%'; if no % show target value). "
        "Prefix each with the scenario `trigger` as brief label: "
        "'Bull Q2 DC >$6B +19% (P=30%) | Base In-line 0% (P=50%) | Bear Capex pause -17% (P=20%)'. "
        "Probs must sum to 100%.\n"
